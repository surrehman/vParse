 ...
 * =====================================================================================
 *
 *       Filename:  CLAVerify.c
 *
 *    Description:  Read & compile input verilog into a forest of DAGs, convert
 *                  it to structural verilog for CVE verification
 *              
 *                  
 *
 *        Version:  1.0
...
void displayUsage(void){
    printf("-I- cla_verify: Read & compille input verilog into a forest of \n");
    printf("                DAGs and convert it back to structural verilog \n");
    printf("                for CVE verification\n");
    printf("    Build time: %s\n", buildTime);
    printf("-I- Usage:\n");
...
int main(int argc, char **argv){
    ...
    for (i=0;i<mList->index;i++){
        ...
        // --------------------------------------------------------------------
        strcat(mpntr->name, "_DAG");
        sprintf(ofile, "%s.v", mpntr->name);
        module2Forest2(mpntr, 0,1,0,ofile,NULL, ofile); // for verification
        // --------------------------------------------------------------------
        //                                                   free() used memory 
        // --------------------------------------------------------------------
        moduleFree(mpntr, 0);
